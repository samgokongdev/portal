...
@section('konten')
  ...
  <div class="w-full p-4 bg-gray-100 shadow rounded-lg">
    <div class="font-bold">Edit Surat Masuk </div>
    <div class="flex space-x-5">
      <div class="w-4/6 space-y-2">
        <form method="post" action="{{ route('suratmasuk.update', $id) }}">
          @csrf
          @method('PUT')
          ...
          <div class="form-control">
            ...
            <input type="text" class="input input-bordered input-sm" name="asal" value="{{ $suratmasuk->asal }}"
              required />
          </div>
          ...
          <div class="form-control">
            ...
            <textarea class="textarea rounded-xl textarea-bordered" name="perihal" required>{{ $suratmasuk->perihal }}</textarea>
          </div>
...
@section('script')
  <script>
    $(function() {
      ...
      $('#datepicker').datepicker("setDate", "<?php echo $suratmasuk->tanggal; ?>");
    });
  </script>
